...
public class MsgEvent extends ListenerAdapter {
    ...
    public void onMessageReceived(MessageReceivedEvent event) throws StringIndexOutOfBoundsException{
        if (event.isFromType(ChannelType.PRIVATE)) {
            ...
            System.out.println("getChannelType : "+event.getChannelType() +" event.getMessage().getChannelType() : "+event.getMessage().getChannelType());
            System.out.println("getChannelName :"+ event.getMember().getVoiceState().getChannel().getName());
            //            System.out.println("audioCh : "+audioCh.toString() +" audioRiv.canReceiveUser : "+audioRiv.canReceiveUser());
        }
